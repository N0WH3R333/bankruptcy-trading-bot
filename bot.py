...
🔥 Популярные вопросы:
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "ask_question":
            ...
            # Принудительно перенаправляем пользователя в канал
            try:
                # Отправляем сообщение пользователю о перенаправлении
                await query.edit_message_text(
                    "📚 Перенаправляем вас в канал с базой знаний...\n\n"
                    "Там вы найдете:\n"
                    "• Актуальную информацию о торгах\n"
                ...
                # Отправляем сообщение в канал, которое уведомит пользователя
                channel_message = f"👋 Пользователь {query.from_user.first_name or 'Аноним'} перешел в канал из бота!"
                
                # Отправляем сообщение в канал
                await context.bot.send_message(
                    chat_id=KNOWLEDGE_CHANNEL_ID,
                    text=channel_message
                )
                
                # Отправляем пользователю сообщение с приглашением в канал
                await context.bot.send_message(
                    chat_id=user_id,
                    text="✅ Вы были перенаправлены в канал!\n\n"
                         "📚 В канале вы найдете всю необходимую информацию по торгам по банкротству.\n\n"
                         "🔙 Для возврата в меню бота нажмите /start",
                ...
                logger.error(f"Ошибка перенаправления в канал: {e}")
                await query.edit_message_text(
                    "❌ Произошла ошибка при перенаправлении в канал.\n\n"
