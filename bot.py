...
🔥 Популярные вопросы:
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "ask_question":
            ...
            # Перенаправляем пользователя в канал
            try:
                ...
                # Отправляем пользователю сообщение с прямой ссылкой на канал
                # Используем tg:// ссылку для автоматического открытия канала
                channel_url = f"tg://resolve?domain=c/{KNOWLEDGE_CHANNEL_ID[4:]}"
                
                # Отправляем сообщение с кнопкой, которая автоматически откроет канал
                await context.bot.send_message(
                    chat_id=user_id,
                    text="📚 Переходим в канал с базой знаний!",
